...
InstructorSchema.statics.addDiscount = async function (
  instructor_id,
  name,
  percentage,
  start_date,
  ...
  if (!discount) throw Error("Discount not created");
...
InstructorSchema.statics.isTeachCourse = async function (
  instructor_id,
  course_id
) {};
InstructorSchema.statics.changeBiography = async function (_id, newBiography) {
  if (!_id || !newBiography) throw Error("All fields must be filled");

  const user = await this.findOne({
    _id,
  });

  if (!user) throw Error("Incorrect id / User not Found");
